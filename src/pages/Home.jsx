  ...
  function filter(category){
    if(category==="All"){
      setCate(food_items);
        ...
        const newList = food_items.filter((items) => items.food_category === category);
       setCate( newList);
      }
  ...
  return (
    ...
    <div className="bg-slate-200 w-full min-h-screen">
      ...
      <div className='w-full flex flex-wrap gap-5 px-5 justify-center items-center pt-8 pb-8'>
        {cate.length>1? cate.map((item)=>(<Card  name={item.food_name} image={item.food_image}
        price={item.price} id={item.id} type={item.food_type}/>)):<div className="text-center text-2xl text-green-500 font-semibold pt-5">No dish Found</div>}
...
{items.map((item)=>(<Card2 name={item.name} price={item.price} image={item.image} id={item.id} qty={item.qty}/>))}
